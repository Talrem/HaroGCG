...
function mulligan(deck, hand){

}

function drawCard(deck, amount) {
	let cards = new Array()
	for (i = 0; i < amount; i++) {
		cards[i] = deck.pull();
	}
...
function initiateArray(array, nbJ) {
	for (i = 0; i < nbJ; i++) {
		array[i] = new Array();
	}
	return array;
...
module.exports = {
	...
	async execute(interaction) {
		...
		switch (step) {
			case -1:
				//Every player draws 5 cards. They can Mulligan once if they want.
				console.log("Start Phase");
			case 0:
				//Activate all of your rested cards, gain 1 Resource card.
				console.log("Resource phase");
			case 1:
				//The turn player draws one card from the top of their deck.
				console.log("Draw Phase");
			case 2:
				//The turn player can play Bases, Units, Pilots, Commands.
